%function [explained_variance]=aggregate_LASSO_test(num_train_1)
clear
tic
num_test_1=30;
num_train_1=170;
...
% feature selection
lambda=[10^-6 10^-5 10^-4 0.001 0.01 0.1 1 10 100 1000];
[B,fitinfo] = lasso(train_data,train_Y,'Standardize',false,'CV',5,'Lambda',lambda,'MaxIter',1e3);

idxLambdaMinMSE = find(fitinfo.MSE==min(fitinfo.MSE));
coef = B(:,idxLambdaMinMSE(1));
coef0 = fitinfo.Intercept(idxLambdaMinMSE(1));
%% error calculation
